...
from app.gui.styles import CARD_TITLE_STYLE, MUTED_TEXT_STYLE, RESULT_FAILURE_STYLE, RESULT_IDLE_STYLE, RESULT_RUNNING_STYLE, RESULT_SUCCESS_STYLE, STEP_BADGE_STYLE, style_button, style_card
...
class ApkInstallPanel(QFrame):
    ...
    def __init__(self):
        ...
        step1 = QLabel("1 选择 APK")
        step1.setStyleSheet(STEP_BADGE_STYLE)
        layout.addWidget(step1)

        self.drop_hint = QLabel("拖拽一个或多个 APK 到这里，或点击选择 APK / 选择文件夹。文件名类似 1.apk(1).1 会自动识别为 APK。")
        self.drop_hint.setWordWrap(True)
        self.drop_hint.setMinimumHeight(58)
        self.drop_hint.setStyleSheet(
            "font-size: 14px; font-weight: 700; color: #174ea6; padding: 12px;"
            "border: 1px dashed #9bbcff; background: #f4f8ff; border-radius: 7px;"
        )
        layout.addWidget(self.drop_hint)

        actions = QHBoxLayout()
        self.choose_button = QPushButton("选择 APK")
        ...
        style_button(self.choose_button, "primary", "选择要安装的 APK 文件。")
        style_button(self.choose_many_button, "secondary", "一次选择多个 APK 加入待安装列表。")
        style_button(self.choose_folder_button, "secondary", "扫描文件夹中的 APK 并加入待安装列表。")
        style_button(self.install_button, "secondary", "把当前 APK 安装到设备连接中心当前选中的设备。")
        style_button(self.clear_button, "secondary", "清空当前页面显示。")
        style_button(self.open_button, "secondary", "打开 APK 安装输出目录。")
        style_button(self.start_install_button, "success", "按待安装 APK 和已勾选设备串行执行安装。")
        self.install_button.setEnabled(False)
        self.install_button.hide()
        self.start_install_button.setEnabled(False)
        for button in [self.choose_button, self.choose_many_button, self.choose_folder_button, self.install_button, self.clear_button, self.open_button]:
            actions.addWidget(button)
        layout.addLayout(actions)

        target_title = QLabel("2 选择安装目标")
        target_title.setStyleSheet(STEP_BADGE_STYLE)
        layout.addWidget(target_title)

        self.target_summary = QLabel("未发现设备。请先在“快速诊断”页检测数据线设备或扫描同一网络。")
        ...
        style_button(self.select_debuggable_targets_button, "secondary", "勾选所有状态为已可调试的设备。")
        style_button(self.clear_targets_button, "secondary", "清空目标设备选择。")
        style_button(self.install_targets_button, "success", "把当前 APK 串行安装到勾选的设备。")
        style_button(self.retry_failed_targets_button, "secondary", "仅重试上次安装失败的设备。")
        ...
        target_actions.addWidget(self.start_install_button)
        ...
        self.target_device_table.setMinimumHeight(140)
        self.target_device_table.setStyleSheet(
            ...
            "QHeaderView::section { background: #f8fafc; color: #374151; font-weight: 700; padding: 7px; border: none; border-right: 1px solid #e5eaf2; }"
        ...
        queue_title = QLabel("待安装 APK")
        queue_title.setStyleSheet(STEP_BADGE_STYLE)
        ...
        style_button(self.start_batch_button, "success", "按队列顺序串行安装所有 APK。")
        style_button(self.retry_failed_button, "secondary", "仅把失败的 APK 重新加入队列并安装。")
        style_button(self.export_result_button, "secondary", "导出批量安装结果，便于发给工程师。")
        ...
        step2 = QLabel("3 确认 APK 信息与安装选项")
        step2.setStyleSheet(STEP_BADGE_STYLE)
        ...
        step3 = QLabel("4 安装结果")
        step3.setStyleSheet(STEP_BADGE_STYLE)
        ...
        output_title.setStyleSheet("font-weight: 700; color: #202124;")
    ...
    def choose_file(self):
        path, _ = QFileDialog.getOpenFileName(self, "选择 APK 文件", "", "APK 文件 (*.apk *apk*);;所有文件 (*.*)")
        if path:
    ...
    def refresh_target_summary(self, *_):
        ...
        if not total:
            self.target_summary.setText("未发现设备。请先在“快速诊断”页检测数据线设备或扫描同一网络。")
        else:
