...
- 提供截屏、10 秒录屏、实时 logcat、adb push、adb pull、adb root、adb remount、无线调试配对等常用 FAE 功能。
...
4. 点击“检测设备”。如果提示未授权，请在设备上点击“允许 USB 调试”。
...
USB 已连接设备时，可点击“开启网络 ADB”，执行：

```text
adb tcpip 5555
```

然后输入设备 IP 和端口 `5555`，点击“ADB 远程连接”。请确认电脑和设备在同一局域网，且防火墙未阻断连接。

Android 11+ 无线调试可使用“ADB 远程配对”，输入配对 IP、配对端口和配对码后执行：

```text
adb pair IP:PORT PAIRING_CODE
```

配对成功后，再使用“ADB 远程连接”连接设备。
