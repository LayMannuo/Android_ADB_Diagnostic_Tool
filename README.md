...
这是一个面向普通客户和 FAE 工程师的 Windows 桌面 ADB 诊断工具。客户不需要理解命令行，只需要连接 Android 设备，在“设备连接”页点击“检测数据线设备”，再进入“快速诊断”点击“一键生成诊断包”，即可得到可发送给工程师分析的 zip 文件。
...
- 提供设备连接中心、网络连接、网段扫描、批量 APK 安装队列、截屏、10 秒录屏、实时 logcat、adb push、adb pull、adb root、adb remount、无线调试配对等常用 FAE 功能。
...
1. `数据线连接`：第一次使用先通过数据线连接设备，点击“检测数据线设备”，确认 USB 连接和授权状态。
2. 只有设备列表中显示“已可调试”后，投屏、日志、截图、文件传输、APK 安装和 ADB 调试窗口才会针对该设备执行。
3. `网络连接`：已知设备 IP 时，输入 IP 和端口后点击“连接设备”；Android 11+ 可先在该区域完成无线配对。
4. `网段扫描`：不知道设备 IP 时，先扫描当前网段，也可以填写起始 IP、结束 IP 和端口后点击“扫描指定范围”。
5. 扫描到的地址只有通过 ADB 验证后才会显示“已可调试”；端口有响应但未通过 ADB 验证时，会显示为“候选设备”。

数据线连接只负责检测 USB、本机 ADB 和授权状态，不展示 IP 或端口。网络连接和网段扫描默认端口为 `5566`，可按现场配置修改。
...
- 目标设备来自“设备连接”页；通过数据线连接、网络连接或网段扫描发现设备后会自动同步到 APK 安装页。
...
- 新增独立“设备连接”页，连接方式拆分为数据线连接、网络连接和网段扫描，设备列表明确区分已可调试、未授权、离线和候选设备。
...
- 新增设备连接中心、网络连接、网段扫描、默认 `5566` 端口、网络范围管理和无线调试配对。
...
- 程序已包含设备连接中心、网络连接、网段扫描、ADB 诊断、单项日志/问题分析、ADB 调试窗口、投屏、截图、录制屏幕和一键诊断包等功能入口。
